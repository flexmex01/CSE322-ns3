...
{
    NS_LOG_FUNCTION(this << tcpHeader);
    //std::cout << "Advertised Window: " << tcpHeader.GetWindowSize() << std::endl;
...
TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
    ...
    // Guard: CopyFromSequence returns nullptr when maxSize==0 (zero-window probe /
    // pure-ACK path).  TcpRateLinux::SkbSent dereferences the pointer unconditionally,
    // so skip rate-ops bookkeeping for these zero-payload packets.
    if (outItem == nullptr)
    {
        return 0;
    }
