...
TcpL4Protocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::TcpL4Protocol")
            ...
            .AddAttribute("SocketClass",
                        "Class of TCP socket objects to instantiate. Must be TcpSocketBase "
                        "or a subclass. Separate from SocketType which selects the "
                        "congestion algorithm.",
                        TypeIdValue(TcpSocketBase::GetTypeId()),
                        MakeTypeIdAccessor(&TcpL4Protocol::m_socketClassId),
                        MakeTypeIdChecker());
    return tid;
}
...
TcpL4Protocol::TcpL4Protocol()
    : m_endPoints(new Ipv4EndPointDemux()),
      ...
      m_socketClassId(TcpSocketBase::GetTypeId())  // add this line
{
...
TcpL4Protocol::CreateSocket(TypeId congestionTypeId, TypeId recoveryTypeId)
{
    ...
    ObjectFactory socketClassFactory;
    socketClassFactory.SetTypeId(m_socketClassId);
    Ptr<TcpSocketBase> socket = socketClassFactory.Create<TcpSocketBase>();
